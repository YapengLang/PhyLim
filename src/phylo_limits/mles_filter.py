...
def make_model_app(
    name, opt_args=None, het=True, tree=None
    ...
    if het == True:
        time_het = "max" if name not in {"JC69", "F81"} else None
    else:
        time_het = None
...
def fit_core_E(algn: AlignedSeqsType, name: str) -> model_result:
    ...
    with open(
        f"/Users/yapenglang/repos/YapengAnalysis/data/data_duchene2022/empiricalDataLoci/Espeland_2018/paralinear_tree_8Sep/{algn.name}.pickle",
        "rb",
    ) as handle:
        m = pickle.load(handle)
    tree = DistanceMatrix(m).quick_tree()

    # fit a time-homo model to the algn
    model = make_model_app(name, opt_args=None, het=False, tree=tree)

    return model(algn)


@define_app
def fit_core_W(algn: AlignedSeqsType, name: str) -> model_result:
    # get tree for each locus
    with open(
        f"/Users/yapenglang/repos/YapengAnalysis/data/data_duchene2022/empiricalDataLoci/Wiens_2012/paralinear_tree_9Sep/{algn.info['source']}.pickle",
        "rb",
    ) as handle:
        m = pickle.load(handle)
    tree = DistanceMatrix(m).quick_tree()
